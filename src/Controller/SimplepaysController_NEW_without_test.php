...
class SimplepaysController extends AppController{
    ...
    public function back() {
		...
		// ------------------
		$f = print_r($back, true);		
		$file = fopen("/var/www/saghysat/logs/simplepay/BACK_" . date("Ymd_His") . ".log","w");
		echo fwrite($file, $f );
		fclose($file);		
		// ------------------
		...
		if($back['raw']['e'] == 'SUCCESS' || $back['raw']['e'] == 'FAIL' || $back['raw']['e'] == 'TIMEOUT' || $back['raw']['e'] == 'CANCEL'){
			$simplepay->retEvent 		 = $back['raw']['e'];	// SUCCESS, FAIL, TIMEOUT, CANCEL			
		}else{
			$simplepay->retEvent 		 = '* MISSING *';
		}
		...
		switch( $back['raw']['e'] ){
			...
			case 'TIMEOUT':
				...
				if(!empty($errorCode)){
					...
					$message['thx'] 	= 'Időtúllépés! &rarr; Lejárt a rendelkezésre álló idő! Kérjük próbálja később!';
				}
				$message['message'] = 'Sikertelen fizetés!';
				break;
			case 'CANCEL':
